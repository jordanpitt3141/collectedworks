...
double HankEnergyFTacrosscell(double *x,double *h,double *u,double g,int j,double dx)
{
    //so we have h,u at midpoints
    //epsilon and sigma are everywhere
...
double HankEnergySTacrosscell(double *x,double *h,double *u,double g,int j,double dx)
{
    //so we have h,u at midpoints
    //epsilon and sigma are everywhere
...
double HankEnergyTTacrosscell(double *x,double *h,double *u,double g,int j,double dx)
{
    //so we have h,u at midpoints
    //epsilon and sigma are everywhere
    ...
    double fgpe = i3*(fgph*fgph*fgph)*fgpux*fgpux;
    ...
    double sgpe = i3*(sgph*sgph*sgph)*sgpux*sgpux;
    ...
    double tgpe = i3*(tgph*tgph*tgph)*tgpux*tgpux;
...
void HankEnergyall(double *x,double *h,double *u,double g,int n, int nBC,double dx, double *Ham)
{
	int i;
	for(i = nBC; i < n - nBC;i++)
	{
       Ham[i - nBC] = HankEnergyacrosscell(x,h,u,g,i,dx);
	}

}

void HankEnergyallPT(double *x,double *h,double *u,double g,int n, int nBC,double dx, double *HamFT, double *HamST, double *HamTT)
{
	int i;
	for(i = nBC; i < n - nBC;i++)
	{
       	HamFT[i - nBC] = HankEnergyFTacrosscell(x,h,u,g,i,dx);
		HamST[i - nBC] = HankEnergySTacrosscell(x,h,u,g,i,dx);
		HamTT[i - nBC] = HankEnergyTTacrosscell(x,h,u,g,i,dx);
	}
