...
diffs = [10.0]
sdirbase = "../../../../../data/postprocessing/uhcomp30sPAPNANEW1/DB/o3/"
#cwdirbase = "../../../../../data/raw/longcontactdiscdx9diff10fileio/o3/9/0/"
cwdirbase = "../../../../../data/raw/Joebigsmooth/o3/9/12/"

for diff in diffs:

    #full crop front front back middez middle zz
    #hylims = [[0.2,2.2],[1.25,1.5],[0.2,2.2] ,[1.31,1.43]]
    #uylims = [[0.0,2.2],[1.25,1.5],[0.2,2.2] ,[1.31,1.43]]
    #xlims = [[350,650],[425,525],[550,625],[528,536]]

    hylims = [[-1.0,1.0],[-0.2,0.2],[-1.0,1.0],[-0.06,0.06]]
    uylims = [[-1.0,1.0],[-0.2,0.2],[-1.0,1.0],[-0.06,0.06]]
    xlims = [[350,650],[425,525],[550,650],[527,537]]    
    ...
    #for l in range(3):
        ...
        igap = int(gap)
        xbeg = int((cxlim[0])/dx) - 1
        xend = int((cxlim[1])/dx) + 1
        if(xbeg < 0):
            xbeg = 0
        if(xend > len(x)):
            xend = len(x)
        ...
        if(l==1):    
            ...
            + "ytick={-0.15,-0.05,0.05,0.15}, \n"\
            + "extra y ticks = {-0.2,-0.1,0.0,0.1,0.2}, \n"\
            + "scaled y ticks=false, \n clip mode=individual,\n  "\
            + "xmin=" + str(cxlim[0] ) + ", \n xmax=" + str(cxlim[1]) + ", \n ymin =" + str(cuylim[0] ) + ", \n  ymax = " + str(cuylim[1] ) + ",\n"\
            + "xlabel=$x$ ($m$), \n ylabel=$u - u_2$ ($m$/$s$)]"\
            + "\\addplot [green!80!black] table {uDB.dat}; \n"\
        ...
        if(l==3):    
            ...
            + "ytick={-0.05,-0.03,-0.01,0.0,0.01,0.03,0.05}, \n"\
            + "extra y ticks = {-0.06,-0.04,-0.02,0.0,0.02,0.04,0.06}, \n"\
            + "scaled y ticks=false, \n clip mode=individual,\n  "\
            + "xmin=" + str(cxlim[0] ) + ", \n xmax=" + str(cxlim[1]) + ", \n ymin =" + str(cuylim[0] ) + ", \n  ymax = " + str(cuylim[1] ) + ",\n"\
            + "xlabel=$x$ ($m$), \n ylabel=$u - u_2$ ($m$/$s$)]"\
            + "\\addplot [green!80!black] table {uDB.dat}; \n"\
