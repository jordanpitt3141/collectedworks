...
for ip in range(len(wdirords)):
    ...
    for l in range(len(ylims)):
        ...
        gap = gaps[l]
        wdir = "../../../../data/raw/Cserre/solitonint/collnonlindx0p1N/"  +wdirord +"/"
        sdir = "../../../../data/postprocessing/Cserre/solitonint/collnonlindx0p1N/"  +wdirord +"/"
        ...
        with open(s,'r') as file1:
             ...
             for row in readfile:       
                 if (j >= 0):
                    ...
                    u.append(float(row[7])) #5 for FDcent, 6 otherwise
                 j = j + 1
             igap = int(gap)
             if zoom[l]:
                 xbeg = int((cxlim[0]+100)/dx)
                 xend = int((cxlim[1]+100)/dx)
