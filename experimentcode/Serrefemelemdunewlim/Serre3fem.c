...
void evolve(double *G, double *h, double *u, double g, double r, double epsilon, double dx, double dt,int n, int nBC, double *nh, double *nG)
{
    ...
    double Gdelip3h = G[i+2] - G[i+1];
    double hdelip3h = h[i+2] - h[i+1];

    double Gip1l = G[i+1] - 0.5*Cadalim(Gdelip3h, Gdeliph , r, epsilon, dx)*Gdeliph;
    double hip1l = h[i+1] - 0.5*Cadalim(hdelip3h, hdeliph , r, epsilon, dx)*hdeliph;
    ...
    for (i = nBC; i < n + nBC;i++)
    {
        ...
        Gdelip3h = G[i+2] - G[i+1];
        hdelip3h = h[i+2] - h[i+1];

        Gip1l = G[i+1] - 0.5*Cadalim(Gdelip3h, Gdeliph , r, epsilon, dx)*Gdeliph;
...
void evolvewrap(double *Ga, double *ha, double *Gabeg, double *Gaend, double *habeg, double *haend, double *hmbeg, double *hmend, double *uebeg, double *ueend, double *umbeg, double *umend, int nfcBC, int nGsBC, double g, double r, double epsilon, double dx, double dt, int n, int nBCa, int nBCm)
{
...
//######################################### SECOND ITERATION #############################
    ...
    evolve(Gabc,habc,uebc,g,r,epsilon,dx,dt,n,cnBC,nhap,nGap);
...
//######################################### THIRD ITERATION #############################
    ...
    evolve(Gabc,habc,uebc,g,r,epsilon,dx,dt,n,cnBC,nhappp,nGappp);
