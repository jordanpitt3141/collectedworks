...
void evolveBCSPONGE(double *h,double *G,double *bed,double *u0,double *u1,double *h0,double *h1 , double *G0, double *G1,double *b0,double *b1, int n, int nBC, int nBCn,double dx, double *nhbc, double *nGbc, double *nubc, double *nbbc)
{
...
void evolvesponge(double *xbc,double xsl, double xsu,double a,double hbase,double *hbc,double *Gbc, double *hblank,double *Gblank,double *bedbc, double *ubc ,double g,double theta, int n, int nBCn,double dx,double dt)
{
	...
	for(i = nBCn; i < n + nBCn ; i++)
    {
		...
		/*if(xbc[i]  > xsl)
		{
			...
			fGel = Gel*uel + 0.5*g*hel*hel;
			fGer = Ger*uer + 0.5*g*her*her;
			...
			Gblank[i - nBCn] = Gbc[i] - dt*idx*(foG -fiG);
		}
		else
		{*/
			// hil and hir
		...
		//printf("hblank[%d] : %2.12f  | hbc[%d] : %2.12f \n",i,hblank[i - nBCn],i,hbc[i]);
...
void evolvewrapBCSponge(double *G,double *h,double *bed,double *h0,double *h1,double *u0,double *u1, double *G0, double *G1,double *h0h,double *h1h,double *u0h,double *u1h, double *G0h, double *G1h,double *b0,double *b1, double g,double dx,double dt, int n, int nBC, int nBCn,double theta, double *hbc, double *Gbc, double *ubc, double *xbc, double xsl, double xsu, double a, double hbase,double max)
{ 
...
//UNFINISHED~~~
    ...
    //update h' and G' evolveBCSPONGE
	evolveBCSPONGE(h,G,bed,u0,u1,h0,h1,G0,G1,b0,b1,n,nBC,nBCn,dx,ihbc, iGbc, iubc, ibbc);
	evolvesponge(xbc,xsl,xsu,a,hbase,ihbc,iGbc, hp,Gp,ibbc,iubc,g,theta,n,nBCn,dx,dt);


	evolveBCSPONGE(hp,Gp,bed,u0h,u1h,h0h,h1h,G0h,G1h,b0,b1,n,nBC,nBCn,dx,ihbc, iGbc, iubc, ibbc);
	evolvesponge(xbc,xsl,xsu,a,hbase,ihbc,iGbc, hpp,Gpp,ibbc,iubc,g,theta,n,nBCn,dx,dt);
	...
	for(i = 0; i < n ; i++)
	{
		...
		/*if(xbc[i]  > xsl)
		{
			//mu =exp( ( pow(2.0,-(xbc[i] - xsl)*idx) - pow(2.0,-(xsu - xsl)*idx) )*log(a) );
			//mu = 1 + pow((xbc[i] - xsl)*idx,3.0);
			//mu = 1.0;
			//h[i] = (0.5*(h[i] +hpp[i]) - hbase) / mu + hbase;
			//G[i] = 0.5*((G[i] +Gpp[i]) / mu /mu);
