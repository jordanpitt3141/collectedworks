...
void evolvesponge(double *hbc,double *Gbc, double *hblank,double *Gblank,double *bedbc, double *ubc ,double g,double theta, int n, int nBCn,double dx,double dt)
{
	...
	for(i = nBCn; i < n + nBCn ; i++)
    	{
			...
			th = hbc[i];
			tu = ubc[i];
			tux = idx*0.5*(ubc[i+1] - ubc[i-1]);
			tbx = idx*(bir - bil);
			tbxx = idx*idx*(bedbc[i+1] - 2*bedbc[i] + bedbc[i-1]);
		
			sourcer = g*0.5*(hihm*hihm - hir*hir);
			sourcec = dx*(-g*th*tbx +  0.5*th*th*tu*tux*tbxx - th*tu*tu*tbx*tbxx);
...
void evolvewrapBCSponge(double *G,double *h,double *bed,double *h0,double *h1,double *u0,double *u1, double *G0, double *G1,double *h0h,double *h1h,double *u0h,double *u1h, double *G0h, double *G1h,double *b0,double *b1, double g,double dx,double dt, int n, int nBC, int nBCn,double theta, double *hbc, double *Gbc, double *ubc)
{ 
...
//UNFINISHED~~~
	...
	evolvesponge(ihbc,iGbc, hp,Gp,ibbc,iubc,g,theta,n,nBCn,dx,dt);


	evolveBCSPONGE(hp,Gp,bed,u0h,u1h,h0h,h1h,G0h,G1h,b0,b1,n,nBC,nBCn,dx,ihbc, iGbc, iubc, ibbc);
	evolvesponge(ihbc,iGbc, hpp,Gpp,ibbc,iubc,g,theta,n,nBCn,dx,dt);
