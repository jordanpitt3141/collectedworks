...
void ufromGh(double *G, double *h,double *hbeg,double *hend,double *ubeg,double *uend, double dx , int n, int nBC, double *ue)
{
    int m = 2*n + 1;
    double idx = 1.0 / dx;
...
void evolvewrap(double *Ga, double *ha, double *Gabeg, double *Gaend, double *habeg, double *haend, double *hmbeg, double *hmend, double *uebeg, double *ueend, double *umbeg, double *umend, int nfcBC, int nGsBC, double g, double dx, double dt, int n, int nBCa, int nBCm)
{
...
//############################### FIRST ITERATION #######################################
    ...
    ufromGh(Gabc,habc,hmbeg+(nBCm -cnBC),hmend,umbeg+(nBCm -cnBC),umend,dx,n,cnBC,ue);
    ...
    evolve(Gabc,habc,uebc,g,dx,dt,n,cnBC,nha,nGa);
...
//######################################### SECOND ITERATION #############################
    ...
    evolve(Gabc,habc,uebc,g,dx,dt,n,cnBC,nhap,nGap);
...
//######################################### THIRD ITERATION #############################
    ...
    evolve(Gabc,habc,uebc,g,dx,dt,n,cnBC,nhappp,nGappp);
